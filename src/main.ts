...
export class RiedelRSP1232HLInstance extends InstanceBase<DeviceConfig> {
  ...
  private handleMessage(message: string): void {
    try {
      ...
      if (topic === "/NetworkStatus/FetchNetworkStatusResponse") {
        ...
        this.log(
          "info",
          `Network settings received: ${this.networkSettings ? "OK" : "null"}`,
        );
        ...
        if (body.alarmHistory) {
          ...
          this.log(
            "info",
            `Alarm history received: ${this.alarmHistory.length} entries`,
          );
        }
        ...
        this.log(
          "info",
          `NMOS enabled: ${this.nmosEnabled}, status: ${this.nmosStatus}`,
        );
  ...
  public async setIpAddress(
    ...
    const updatedSettings = JSON.parse(
      JSON.stringify(this.networkSettings),
    ) as NetworkSettings;
    const targetInterface = updatedSettings.networkInterfaceSettings.find(
