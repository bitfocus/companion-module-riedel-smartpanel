...
export function getFeedbacks(
  instance: RiedelRSP1232HLInstance,
  ...
  return {
    ...
    healthStatus: {
      ...
      name: "Health Status",
      description:
        "Change button style based on device health (OK, Warnings, Errors)",
      defaultStyle: {
        color: 0xffffff,
        bgcolor: 0x00ff00,
      },
    ...
    alarmCountDisplay: {
      ...
      callback: (feedback) => {
        ...
        const bgcolor =
          count > 0
            ? (feedback.options.hasAlarmColor as number)
            : (feedback.options.noAlarmColor as number);
        const text = (feedback.options.format as string).replace(
          "{count}",
          String(count),
        );
        return {
    ...
    ptpStatusDisplay: {
      ...
      callback: (feedback) => {
        ...
        if (status === "TimeReceiverLocked") {
          ...
          text = (feedback.options.shortText as boolean)
            ? "PTP\\nLocked"
            : "TimeReceiverLocked";
